...
export default function ClockPage(){
  ...
  function connect(){ const ws = new WebSocket(wsUrl + `?room=${encodeURIComponent(room)}`); ws.onopen=()=>setSocket(ws); ws.onclose=()=>setSocket(null) }
  ...
  function addClock(){ const id = Math.random().toString(36).slice(2,10); setClocks(p=>[...p,{id,name:'Nuovo Clock',segments:6,fill:0,visible:true}]) }
  function update(id:string, d:number){ setClocks(p=>p.map(c=>c.id===id?{...c, fill: Math.max(0, Math.min(c.segments, c.fill+d))}:c)) }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center"><h2 className="text-xl font-semibold">Clock</h2><button className="btn" onClick={addClock}>+ Aggiungi</button></div>
        ...
        {clocks.map(c=>(
          <div key={c.id} className="card">
            <div className="flex items-center justify-between mb-2">
              ...
              <label className="label flex items-center gap-2"><input type="checkbox" checked={c.visible} onChange={e=>setClocks(v=>v.map(x=>x.id===c.id?{...x,visible:e.target.checked}:x))}/>Visibile</label>
            </div>
            ...
            <div className="mt-3 grid grid-cols-6 gap-1">{Array.from({length:c.segments}, (_,i)=>i<c.fill).map((on,i)=>(<div key={i} className={`h-2 rounded ${on?'bg-teal-500':'bg-zinc-800'}`} />))}</div>
          </div>
        ))}
      </div>
    </div>
